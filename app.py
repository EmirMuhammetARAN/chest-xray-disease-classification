...
def generate_gradcam_plot(image_pil, heatmap, title):
    ...
    ax.set_title(
        title, fontsize=11, fontweight="bold", color="white", backgroundcolor="darkred"
    )
    ax.axis("off")
    return fig
...
def predict_xray(image):
    ...
    df = df.sort_values(
        by="Olasılık", ascending=False, key=lambda x: x.str.strip("%").astype(float)
    )
    ...
    try:
        for idx, (name, score, class_idx) in enumerate(top_3):
            ...
            heatmap = make_gradcam_heatmap(
                img_array, model, LAST_CONV_LAYER, pred_index=class_idx
            )
...
examples_list = []
if os.path.exists("example_1.jpg"):
    examples_list.append(["example_1.jpg"])
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            output_table = gr.Dataframe(
                label="Tüm Sonuçlar (En Düşükler Dahil)",
                headers=["Hastalık", "Olasılık"],
                interactive=False,
            )
